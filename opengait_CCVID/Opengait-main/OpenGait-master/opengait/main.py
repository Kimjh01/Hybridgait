...
def run_model(cfgs, training):
    ...
    model_cfg = cfgs['model_cfg']
        ...
        # class_num을 TRAIN_SET 기준으로 자동 설정
    if training and 'SeparateBNNecks' in model_cfg and 'class_num' in model_cfg['SeparateBNNecks']:
        try:
            partition_path = cfgs['data_cfg']['dataset_partition']
            with open(partition_path, 'r') as f:
                partition = json.load(f)
            num_classes = len(partition['TRAIN_SET'])
            model_cfg['SeparateBNNecks']['class_num'] = num_classes
            msg_mgr.log_info(f"[INFO] Automatically set class_num to {num_classes} from TRAIN_SET.")
        except Exception as e:
            msg_mgr.log_info(f"[WARNING] Failed to set class_num automatically: {e}")
