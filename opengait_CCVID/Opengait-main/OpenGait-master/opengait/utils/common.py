...
def config_loader(path):
    ...
    with open("Opengait-main/Opengait-master/configs/default.yaml", 'r') as stream:
...
def get_ddp_module(module, find_unused=False):
    """
    ① 분산 초기화가 안 돼 있으면 그냥 return module
    ② 돼 있으면 DistributedDataParallel 래핑
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    module = module.to(device)

    if not (dist.is_available() and dist.is_initialized()):
        return module          # ← 단일 GPU/CPU
