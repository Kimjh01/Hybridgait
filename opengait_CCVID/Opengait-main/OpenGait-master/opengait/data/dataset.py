...
class DataSet(tordata.Dataset):
    ...
    def __loader__(self, paths, target_len=30):
        pose_data, sil_data = None, None
        ...
        for pth in sorted(paths):
            ...
            # pose는 'pose' 또는 'heatmap'이 들어간 이름, 나머지는 silhouette로 처리
            if 'pose' in pth or 'heatmap' in pth:
                pose_data = data
            elif 'sil' in pth:
                    sil_data = data

        if pose_data is None or sil_data is None:
            raise ValueError(f"Missing pose or silhouette data in paths: {paths}")

    # 길이 맞추기
        def pad_or_trim(d):
            if len(d) < target_len:
                return d + [d[-1]] * (target_len - len(d))
            return d[:target_len]

        pose_data = pad_or_trim(pose_data)
        sil_data = pad_or_trim(sil_data)

        return [pose_data, sil_data]
    ...
    def __getitem__(self, idx):
        ...
        if not isinstance(data_list, list) or len(data_list) != 2:
            print(f"❗ WARNING: Invalid data_list format at idx {idx}: {type(data_list)}, len={len(data_list)}")
    ...
    #     seq_info = self.seqs_info[idx]
    #     label_str = seq_info[0]
    #     label = self.pid2label[label_str]  # <- 정수 라벨로 변환

    #     return data_list, label  # <- 수정됨
    ...
    def __dataset_parser(self, data_config, training):
        ...
        # all_labels = sorted(list(set([seq[0] for seq in self.seqs_info])))
        # self.pid2label = {pid: idx for idx, pid in enumerate(all_labels)}
