...
class TransferDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with open(pose_path, "rb") as f:
            pose_data = pickle.load(f)            # (T,17,3)

        # ---------- GREW용 슬라이싱 우회 ----------
        if pose_data.ndim == 2 and pose_data.shape[1] == 53:   # (T,53)일 때만 자름
            pose_data = pose_data[:, 2:].reshape(-1, 17, 3)
        elif pose_data.shape[1] != 17:                         # 17이 아니면 오류
            raise ValueError(f"unexpected joint dim: {pose_data.shape}")

        # ---------- 경로 분리(OS 무관) ----------
        parts = Path(pose_path).parts        # (..., ID, seq, view, file.pkl)
        ...
        # ---------- 저장 ----------
        out_pkl_dir = Path(self.save_root, 'pkl', id_, seq, view)
        out_pkl_dir.mkdir(parents=True, exist_ok=True)
        pickle.dump(heatmap_img, open(out_pkl_dir / parts[-1], "wb"))

        if index < 10:                         # 샘플 이미지 저장
            out_img_dir = Path(self.save_root, 'images', id_, seq, view)
            out_img_dir.mkdir(parents=True, exist_ok=True)
            for t in range(heatmap_img.shape[0]):
                cv2.imwrite(out_img_dir / f'bone_{t}.jpg', heatmap_img[t, 0])
                cv2.imwrite(out_img_dir / f'pose_{t}.jpg', heatmap_img[t, 1])
