...
def scan_directory_structure():
    """디렉토리 구조를 스캔하여 데이터 수집"""
    print("\n=== 디렉토리 구조 스캔 ===")
    
    all_data = []
    
    # src_root 하위의 모든 디렉토리 스캔
    ...
    for session_name in os.listdir(src_root):
        ...
        for folder_name in os.listdir(session_path):
            ...
            # ID 추출 (폴더명에서 숫자 패턴 찾기)
            # 예: "001_01" -> "001", "002_02" -> "002"
            id_match = re.match(r'(\d+)', folder_name)
            if id_match:
                id_info = id_match.group(1)
                path_info = f"{session_name}/{folder_name}"
                all_data.append((path_info, id_info))
                print(f"  발견: {path_info} -> ID: {id_info} (이미지 {len(img_files)}개)")
            else:
                print(f"  Warning: {folder_name}에서 ID를 추출할 수 없습니다.")
    
    print(f"\n전체 발견된 데이터: {len(all_data)}개")
    return all_data

def parse_all_data():
    """모든 데이터를 스캔하고 6:2:2로 분할"""
    ...
    # 각 ID별 데이터 수 출력
    print("\nID별 데이터 분포:")
    for id_info, paths in sorted(id_groups.items()):
        print(f"  ID {id_info}: {len(paths)}개 시퀀스")
